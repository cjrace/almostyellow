import React, { useEffect, useState } from 'react';
import Image from "next/image";
import Link from 'next/link';
import { Button, Group } from '@mantine/core';

export default function Home() {

  return (
    <>
    <h1>Almost yellow</h1>
      <p>This is our homepage, we should make it prettier.</p>

      <script 
        data-trigger="custom"
        src="https://run.confettipage.com/here.js" 
        data-confetticode="" async></script>

        <button onClick ={ () => window.ConfettiPage.play()}> Play confetti!</button>

          <Group justify="center">

            <Button leftSection="🎲" variant="default" component="a" href="/games">
              View our games
            </Button>

            <Button leftSection="🤔" variant="default" component="a" href="/decisionmaker">
              How we make decisions
            </Button>

            <Button leftSection="🥄" variant="default" component="a" href="/recipes">
              Our recipes
            </Button>

            <Button leftSection="🍹" variant="default" component="a" href="/cocktails">
              Cocktail time!
            </Button>

          </Group>

          <br></br>

        <footer >
          <a
            href="https://github.com/cjrace/almostyellow"
            target="_blank"
            rel="noopener noreferrer"
          >
            <Image
              aria-hidden
              src="/icons/github-mark-white.png"
              alt="GitHub icon"
              width={16}
              height={16}
            />
            GitHub
          </a>
          <div>
            <Link href="/admin">Admin area</Link>
          </div>
        </footer>
        </>
        
      );
    }